...
static void ketopt_permute(const char *argv[], int j, int n) /* move argv[j] over n elements to the left */
{
	int k;
	const char *p = argv[j];
	for (k = 0; k < n; ++k)
 ...
 */
static int ketopt(ketopt_t *s, int argc, const char *argv[], int permute, const char *ostr, const ko_longopt_t *longopts)
{
	int opt = -1, i0, j;
	if (permute) {
