...
* *use fast mode* makes the compression process faster and usually leads to a bit smaller archives. The archives are, however, different (but equivalent, which means that store exactly the same data) for different program runs. 
