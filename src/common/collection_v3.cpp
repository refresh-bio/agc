...
void CCollection_V3::add_segments_placed(vector<segments_to_place_t>& segments_to_place)
{
	...
	{
		string sample_name = desc.sample_name;

		if (sample_name.empty())
			sample_name = extract_contig_name(desc.contig_name);
...
// *******************************************************************************************
bool CCollection_V3::get_samples_list(vector<string>& v_samples, bool sorted)
{
	lock_guard<mutex> lck(mtx);
