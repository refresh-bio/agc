...
// *******************************************************************************************
void CCollection_V1::add_segments_placed(vector<segments_to_place_t>& segments_to_place)
{
	lock_guard<mutex> lck(mtx);

	for (auto& seg : segments_to_place)
	{
		auto& vec = add_segment_basic(seg.sample_name, seg.contig_name, seg.group_id, seg.in_group_id, seg.is_rev_comp, seg.data_size);

		if (seg.seg_part_no >= vec.size())
			vec.resize(seg.seg_part_no + 1, segment_desc_t(555555555, 555555555, true, 555555555));

		vec[seg.seg_part_no] = segment_desc_t(seg.group_id, seg.in_group_id, seg.is_rev_comp, seg.data_size);
	}
}
...
// *******************************************************************************************
bool CCollection_V1::get_samples_list(vector<string>& v_samples, bool sorted)
{
	lock_guard<mutex> lck(mtx);
